...
repo_url = "https://github.com/alchemy-fr/Phraseanet"
token = ""
...
def clone_or_update_ghaminer():
    ...
    subprocess.run(["git", "clone", repo_url, str(GHAMINER_PATH)],
...
def run_ghaminer_if_needed(repo_url: str, token: str):
    ...
    # ------
    try:
        subprocess.run([sys.executable, str(GHAMETRICS_PATH),
                        "--s", repo_url,
                        "--token", token],
                       check=True, capture_output=True, text=True)
    except subprocess.CalledProcessError as e:
        print("ERROR!! GHAMiner")
...
if __name__ == "__main__":
    run_ghaminer_if_needed(repo_url,token)
